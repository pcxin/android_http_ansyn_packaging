 ...
 */
public class AnsynHttpRequestV2 {
...
//	}
	...
	private static void doAsynRequest(final int sendType,
		...
		// 请求
		ThreadPoolUtils.execute(new MyRunnableV2(sendType,
				map, context,
				callBack, url,
...
class MyRunnableV2 implements Runnable{
	...
	public MyRunnableV2(final int sendType,
			final Map<String, String> map, final Context context,
			final ObserverCallBack callBack, final String url,
			final boolean isCache, final boolean isShowDialog, final int intUrl){
...
//				httpClient.getCookieStore().getCookies().g
		...
		try { // 回调数据
			if(callBack != null) {
				final String resData = data;
				((Activity)context).runOnUiThread(new Runnable() {    
					public void run() {  
						callBack.back(resData, intUrl);  
					}    
				});
